...
def my_args():
	...
	mainParser.add_argument("-rc", help="read count column name", default="total_guideseq_reads")
	mainParser.add_argument("-off", help="off-target sequence column name", default="target_aligned")
	mainParser.add_argument("-on", help="on-target sequence column name", default="on_target_sequence")
...
def barplot(df,x,y,estimator,outLabel):
	fig, ax = plt.subplots(figsize=(8,4))
	sns.barplot(data=df,x=x,y=y,estimator=estimator,order=['NAA', 'NAC', 'NAG', 'NAT', 'NCA', 'NCC', 'NCG', 'NCT',
 'NGA', 'NGC', 'NGG', 'NGT', 'NTA', 'NTC', 'NTG', 'NTT'],palette ="viridis",capsize=0.3,errorbar="sd")
...
def calculate_enrichment_ratio(df,args):
	...
	df2 = df2[~df2[args.off].str.contains("-")]
...
def display_logo(myList,n,out):
    ...
    os.system(f"weblogo --units probability -c classic --format svg < tmp.txt > {out}.top{n}.svg")
...
def main():
	...
	barplot(df,"PAM","read_percent","sum",args.outLabel)
